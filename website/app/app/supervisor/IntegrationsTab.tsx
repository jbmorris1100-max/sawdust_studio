...
// crew member's phone opens to clock in/out or start/end a break. Scanning
// requires a signed-in crew member: if the device has no valid crew-auth
// session it is sent to the PIN/Face ID sign-in first — the action is always
// attributed to a verified crew member, never a typed name.
...
const STATIONS: Station[] = [
  { label: 'Shop Entrance', desc: 'Scan + sign in to clock IN',  action: 'clock_in' },
  { label: 'Shop Exit',     desc: 'Scan + sign in to clock OUT', action: 'clock_out' },
  { label: 'Breakroom In',  desc: 'Scan + sign in to start break', action: 'break_start' },
  { label: 'Breakroom Out', desc: 'Scan + sign in to end break',   action: 'break_end' },
];

const PRINT_NOTE: Record<string, string> = {
  clock_in:    'Scan + sign in to clock in',
  clock_out:   'Scan + sign in to clock out',
  break_start: 'Scan + sign in to start your break',
  break_end:   'Scan + sign in to end your break',
};
